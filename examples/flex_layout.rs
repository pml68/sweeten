...
use iced::mouse;
use iced::widget::{
    button, canvas, center, checkbox, column as iced_column, container,
    horizontal_rule, horizontal_space, pick_list, row as iced_row, scrollable,
    text, vertical_rule,
};
use iced::{
...
impl Layout {
    fn title(&self) -> String {
        format!("{} - Layout - Iced Flex", self.example.title)
    }
    ...
    fn view(&self) -> Element<Message> {
        ...
        let controls = row([
            (!self.example.is_first()).then_some(Element::from(
                button("← Previous")
                    .padding([5, 10])
...
fn application<'a>() -> Element<'a, Message> {
    let header = container(
        row![square(40), "Header!", square(40),]
            .padding(10)
            .align(Center)
            .justify(JustifyContent::SpaceBetween),
    )
